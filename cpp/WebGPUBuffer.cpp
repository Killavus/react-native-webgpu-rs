...
WebGPUBuffer::WebGPUBuffer() : HybridObject(TAG), buffer_(nullptr), device_(nullptr) {}
WebGPUBuffer::WebGPUBuffer(WGPUBuffer buffer, WGPUDevice device) : HybridObject(TAG), buffer_(buffer), device_(device) {}
...
static void handleMapAsync(WGPUMapAsyncStatus status, WGPUStringView message, void* userdata1, void* userdata2) {
...
std::shared_ptr<Promise<void>> WebGPUBuffer::mapAsync(double mode, std::optional<double> offset, std::optional<double> size) {
  ...
  ThreadPool::shared().run([=, this] {
    ...
    wgpuBufferMapAsync(buffer_, (uint64_t) mode, wgpuOffset, wgpuSize, callbackInfo);
...
std::shared_ptr<ArrayBuffer> WebGPUBuffer::getMappedRange(std::optional<double> offset, std::optional<double> size) {
  auto wgpuOffset = (size_t) offset.value_or(0);
  auto wgpuSize = wgpuBufferGetSize(buffer_) - (size_t) offset.value_or(0);
  
  uint8_t *mappedData = (uint8_t*) wgpuBufferGetMappedRange(buffer_, wgpuOffset, wgpuSize);
