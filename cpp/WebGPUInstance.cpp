...
static void handleRequestAdapter(WGPURequestAdapterStatus status,
  ...
  auto promise = reinterpret_cast<std::shared_ptr<Promise<std::shared_ptr<HybridNitroWGPUAdapterSpec>>>*>(userdata1);
  ...
  switch(status) {
    ...
    case WGPURequestAdapterStatus_Unavailable:
      printf("Unavailable\n");
      promise->get()->reject(std::make_exception_ptr("WebGPU Adapter unavailable"));
      delete promise;
      break;
    case WGPURequestAdapterStatus_InstanceDropped:
      printf("Instance dropped\n");
      promise->get()->reject(std::make_exception_ptr("WebGPU Instance dropped while requesting adapter"));
      delete promise;
      break;
    default:
      return;
...
void WebGPUInstance::enumerateAdapters() {
  ...
  WGPUAdapter *adapters = (WGPUAdapter*) malloc(sizeof(WGPUAdapter) * adapter_count);
  assert(adapters);
...
std::shared_ptr<Promise<std::shared_ptr<HybridNitroWGPUAdapterSpec>>> WebGPUInstance::requestAdapter() {
